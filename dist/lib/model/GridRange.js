...
var GridRange = /** @class */ (function () {
    ...
    GridRange.fromRefs = function (model, cellRefs) {
        ...
        var _a = GridCell_1.GridCell.unmakeRefToArray(cellRefs[0]), loCol = _a[0], loRow = _a[1];
        var _b = [loCol, loRow], hiCol = _b[0], hiRow = _b[1];
        for (var _i = 0, cellRefs_1 = cellRefs; _i < cellRefs_1.length; _i++) {
            var cr = cellRefs_1[_i];
            var _c = GridCell_1.GridCell.unmakeRefToArray(cr), col = _c[0], row = _c[1];
            if (loCol > col)
                loCol = col;
            if (hiCol < col)
                hiCol = col;
    ...
    GridRange.fromPoints = function (model, points) {
        var refs = points.map(function (p) { return GridCell_1.GridCell.makeRef(p.x, p.y); });
    ...
    GridRange.createInternal = function (model, cells) {
        ...
        cells = cells.sort(function (a, b) { return a.ref < b.ref ? -1 : 1; });
        var _a = GridCell_1.GridCell.unmakeRefToArray(cells[0].ref), lc = _a[0], lr = _a[1];
        var _b = GridCell_1.GridCell.unmakeRefToArray(u.last(cells).ref), hc = _b[0], hr = _b[1];
        return new GridRange({
            ltr: cells,
            width: hc - lc,
            height: hr - lr,
