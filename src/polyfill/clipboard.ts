...
export class ClipboardPolyfill {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (seemToBeInIE()) {
        if (writeIE(data)) {
          ...
          reject(
            new Error("Copying failed, possibly because the user rejected it.")
          );
        }
        return;
      }
  ...
  public static read(): Promise<DT> {
    return new Promise((resolve, reject) => {
      ...
      this.readText().then((s: string) => resolve(DTFromText(s)), reject);
    }) as Promise<DT>;
  }
...
function readIE(): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    if (text === "") {
      reject(
        new Error("Empty clipboard or could not read plain text from clipboard")
      );
    } else {
      resolve(text);
    }
